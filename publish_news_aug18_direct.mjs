...
function validateBatch() {
  ...
  for (const article of articles) {
    ...
    if (article.publishedAt.toISOString() !== "2026-08-18T00:00:00.000Z") throw new Error(`Invalid publication date: ${article.slug}`);
  }
}
...
try {
  ...
  await appendFile("/tmp/billionaire-collection-news-upsert-2026-08-18.error.log", `${new Date().toISOString()} ${message}\n`);
