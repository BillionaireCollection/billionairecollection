...
const publicationDate = articles[0]?.publishedAt;
const statement = `
...
try {
  ...
  console.log(JSON.stringify({ success: true, operation: "hostinger_news_upsert", count: articles.length, publishedAt: publicationDate?.toISOString() }));
} finally {
  await pool.end();
}
