...
async function upsertManyNewsArticles(articles3) {
  for (const article of articles3) {
...
async function fetchFromNewsAPI() {
  ...
  for (const q of queries) {
    ...
    for (const a of data.articles) {
      ...
      const publishedAt3 = new Date(a.publishedAt);
      const slug = slugify(a.title, publishedAt3);
...
// publish_news_aug13.mjs
import { writeFile } from "node:fs/promises";
import { SignJWT as SignJWT2 } from "jose";
import superjson2 from "superjson";
var LIVE_ENDPOINT = "https://billionairecollection.com/api/trpc/news.upsertMany";
var publishedAt = /* @__PURE__ */ new Date("2026-08-13T00:00:00.000Z");
var articles = [
  {
  ...
  },
  {
    ...
    title: "Salon Priv\xE9 Expands Its Focus on Modern Hypercar Collecting",
    summary: "Salon Priv\xE9 2026 will introduce new Limited Production Supercars and Hypercar Heaven concours classes at Blenheim Palace. The additions reflect growing collector attention on performance icons from Ferrari, Bugatti, Pagani, Rimac and other modern marques.",
    category: "Automotive",
    source: "Boat International",
  ...
  },
  {
    ...
    category: "Technology",
    source: "Union Bancaire Priv\xE9e",
...
if (process.argv[1]?.endsWith("publish_news_aug13.mjs")) {
  let requireEnv = function(name) {
    const value = process.env[name];
    if (!value) throw new Error(`${name} is not set`);
    return value;
  };
  requireEnv2 = requireEnv;
  try {
    ...
    }).setProtectedHeader({ alg: "HS256", typ: "JWT" }).setIssuedAt(now).setExpirationTime(now + 300).sign(jwtSecret);
    const response = await fetch(LIVE_ENDPOINT, {
      ...
      body: JSON.stringify(superjson2.serialize(articles))
    });
    ...
    await writeFile("/tmp/billionaire-collection-news-upsert-2026-08-13.error.log", `${(/* @__PURE__ */ new Date()).toISOString()} ${message}
`);
...
// publish_news_aug14.mjs
var publishedAt2 = /* @__PURE__ */ new Date("2026-08-14T00:00:00.000Z");
var articles2 = articles.map((article) => ({ ...article, publishedAt: publishedAt2 }));
articles2[3] = {
...
async function startServer() {
  if (shouldPublishAugust14News) {
    try {
      await upsertManyNewsArticles(articles2);
      console.log(`[News] Hostinger production upsert completed for ${articles2.length} articles dated 2026-08-14.`);
    } catch (error) {
