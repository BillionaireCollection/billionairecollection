...
export default function SphereAnimation({ size = 600, className, style }: SphereAnimationProps) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      if (r >= whiteThreshold) {
        if (isMobile) {
          ...
          colors[i * 3]     = 0.45;   // red — much darker gold
          colors[i * 3 + 1] = 0.28;   // green — suppressed for warm gold
          colors[i * 3 + 2] = 0.04;   // blue — minimal
        } else {
          // Desktop: AdditiveBlending — use boosted values for glow
          const goldBright = 0.9 + Math.random() * 0.4;
          col = colorGold.clone();
        ...
        if (isMobile) {
          ...
          const sparkle = 2.2 + Math.random() * 0.5;
          colors[i * 3]     = sparkle;
    ...
    const animate = () => {
      ...
      if (!isMobile) {
        const sparkleBase = 1.8;
        const sparkleRange = 0.9;
        ...
        for (let i = 0; i < COUNT; i++) {
          if (isSparkle[i]) {
            const phase = (i * 0.37 + t * pulseSpeed) % (Math.PI * 2);
            const dotPulse = 0.28 + (Math.sin(phase) * 0.5 + 0.5) * 0.14; // 0.28–0.42
            colAttr.setXYZ(i, dotPulse, dotPulse, dotPulse);
          }
        }
        colAttr.needsUpdate = true;
