...
export default function Admin() {
  ...
  const cardApps = trpc.card.list.useQuery(undefined, { enabled: activeTab === "cards" && isAuthenticated && user?.role === "admin" });
  const goldenApps = trpc.goldenTicket.list.useQuery(undefined, { enabled: activeTab === "golden" && isAuthenticated && user?.role === "admin" });
  const conciergeReqs = trpc.concierge.list.useQuery(undefined, { enabled: activeTab === "concierge" && isAuthenticated && user?.role === "admin" });
  const contactEnqs = trpc.contact.list.useQuery(undefined, { enabled: activeTab === "contact" && isAuthenticated && user?.role === "admin" });
  const newsletterSubs = trpc.newsletter.list.useQuery(undefined, { enabled: activeTab === "newsletter" && isAuthenticated && user?.role === "admin" });
      ...
      <div style={{ padding: "2rem", maxWidth: "1400px", margin: "0 auto" }}>
        ...
        {activeTab === "cards" && (
          <SubmissionsTable
            ...
            data={cardApps.data ?? []}
            columns={["Name", "Email", "Phone", "Country", "Occupation", "Net Worth", "Tier", "Referral", "Status", "Date", "Actions"]}
            renderRow={(row: any) => (
        ...
        {activeTab === "golden" && (
          <SubmissionsTable
            ...
            columns={["Name", "Email", "Phone", "Country", "Referred By", "Message", "Status", "Date", "Actions"]}
            renderRow={(row: any) => (
              <tr key={row.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                <Td>{row.name}</Td>
                <Td>{row.email}</Td>
        ...
        {activeTab === "concierge" && (
          <SubmissionsTable
            ...
            columns={["Name", "Email", "Phone", "Request Type", "Budget", "Preferred Date", "Description", "Status", "Date", "Actions"]}
            renderRow={(row: any) => (
              <tr key={row.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                <Td>{row.name}</Td>
                <Td>{row.email}</Td>
        ...
        {activeTab === "contact" && (
          <SubmissionsTable
            ...
            columns={["Name", "Email", "Phone", "Subject", "Division", "Message", "Status", "Date", "Actions"]}
            renderRow={(row: any) => (
              <tr key={row.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                <Td>{row.name}</Td>
                <Td>{row.email}</Td>
